...
class StorageBox():
    def __init__(self, initial_storage, initial_mass,
                 ...
                 run_NBTC_Q=False, run_NBTC_ET=False,
                 run_NBTC_react=False, run_water_bTTD=False,
        ...
        max_age_tracked : float, optional
            Maximum age tracked in NBTC and backwards TTDs.
        ...
        self.dt = dt
        self.run_NBTC_Q = run_NBTC_Q
        self.run_NBTC_ET = run_NBTC_ET
        self.run_NBTC_react = run_NBTC_react
    ...
    def solve_NBTC_Q(self):
        ...
        NBTC = np.zeros((np.count_nonzero(self.input_mass_flux), num_age_steps), dtype=float)
        idx_mass_inputs = np.nonzero(self.input_mass_flux)[0]

        for j in range(len(NBTC)):
            t = idx_mass_inputs[j]
            S_bar = (
                (5 * self.storage[t:-1]
    ...
    def run(self):
        ...
        if self.run_NBTC_Q is True:
            self.solve_NBTC_Q()

        if self.run_NBTC_ET is True:
            self.solve_NBTC_ET()

        if self.run_NBTC_react is True:
            self.solve_NBTC_react()
