...
class StorageBox():
    def __init__(self, initial_storage, initial_mass,
                 ...
                 run_NTBC_Q=False, run_NTBC_ET=False,
                 run_NTBC_react=False, run_water_bTTD=False,
        ...
        max_age_tracked : float, optional
            Maximum age tracked in NTBC and backwards TTDs.
        ...
        self.dt = dt
        self.run_NTBC_Q = run_NTBC_Q
        self.run_NTBC_ET = run_NTBC_ET
        self.run_NTBC_react = run_NTBC_react
    ...
    def solve_NTBC_Q(self):
        ...
        NTBC = np.zeros((np.count_nonzero(self.input_mass_flux), num_age_steps), dtype=float)
        idx_mass_inputs = np.nonzero(self.input_mass_flux)[0]

        for j in range(len(NTBC)):
            t = idx_mass_inputs[j]
            S_bar = (
                (5 * self.storage[t:-1]
    ...
    def run(self):
        ...
        if self.run_NTBC_Q is True:
            self.solve_NTBC_Q()

        if self.run_NTBC_ET is True:
            self.solve_NTBC_ET()

        if self.run_NTBC_react is True:
            self.solve_NTBC_react()
